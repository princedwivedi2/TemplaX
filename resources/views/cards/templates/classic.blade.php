    ...
    <div style="position: relative; z-index: 1; height: 100%; display: flex; padding: 25px; box-sizing: border-box;">
        ...
        <div style="width: 35%; display: flex; flex-direction: column; align-items: center; justify-content: center; padding-right: 20px;">
            <div style="width: 90px; height: 90px; border-radius: 45px; overflow: hidden; background: white; padding: 3px; box-shadow: 0 10px 20px -5px rgba(0,0,0,0.3);">
                <img id="photo-landscape" src="{{ $logoUrl ?? asset('images/default-profile.svg') }}" alt="Logo" style="width: 100%; height: 100%; object-fit: cover; border-radius: 45px;">
            </div>
            <!-- Social Links -->
            <div style="display: flex; gap: 15px; margin-top: 15px;">
                @if(isset($linkedin) && $linkedin)
                <a href="{{ $linkedin }}" id="linkedin-landscape" style="color: #4cc9f0; text-decoration: none; transition: all 0.3s ease;">
                    <i class="bi bi-linkedin" style="font-size: 18px;"></i>
                </a>
                @endif
                @if(isset($twitter) && $twitter)
                <a href="{{ $twitter }}" id="twitter-landscape" style="color: #4361ee; text-decoration: none; transition: all 0.3s ease;">
        ...
        <div style="flex: 1; display: flex; flex-direction: column; justify-content: center;">
            ...
            <div style="margin-bottom: 15px;">
                <h2 id="name-landscape" style="margin: 0; font-size: 24px; font-weight: 700; background: linear-gradient(90deg, #4361ee, #4cc9f0); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ $full_name ?? 'Your Name' }}</h2>
                <p id="role-landscape" style="margin: 4px 0 0 0; font-size: 14px; color: #4cc9f0; font-weight: 500;">{{ $job_title ?? 'Job Title' }}</p>
                <p id="company-landscape" style="margin: 2px 0 0 0; font-size: 14px; color: #e2e8f0;">{{ $company_name ?? 'Company Name' }}</p>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; font-size: 12px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <span id="email-landscape" style="color: #e2e8f0;">{{ $email ?? 'email@example.com' }}</span>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <i class="bi bi-telephone" style="color: #4cc9f0;"></i>
                    <span id="phone-landscape" style="color: #e2e8f0;">{{ $phone ?? '+1 234 567 890' }}</span>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <i class="bi bi-globe" style="color: #7209b7;"></i>
                    <span id="website-landscape" style="color: #e2e8f0;">{{ $website ?? 'www.example.com' }}</span>
                </div>
                <div style="display: flex; align-items: center; gap: 8px;">
                    <i class="bi bi-geo-alt" style="color: #3a0ca3;"></i>
                    <span id="address-landscape" style="color: #e2e8f0;">{{ $address ?? 'Your Address' }}</span>
                </div>
            </div>
        </div>
    </div>
